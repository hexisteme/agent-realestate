...
## 테스트
```bash
.venv/bin/python -m pytest tests/ -q   # 130 passed (CI 자동 실행)
```
...
## GitHub
- 코드: `github.com/hexisteme/agent-realestate` (PUBLIC, MIT) — push 마다 Actions CI(pytest 130).
- **데이터·개인 운영 레이어는 repo 제외**(.gitignore): 네이버 호가(DB권)·개인 프로필·report/·의사결정 로그·일회성 분석 스크립트. 공공 통계 픽스처(regime 2종)만 포함.
