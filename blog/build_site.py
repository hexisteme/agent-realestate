...
<p>단지 생활 맥락은 최근접 학교·학원 정보, SRTM30m 중심±150m 표고 기반 경사 근사, 커뮤니티 후기의 표본수·긍정/주의 테마로 제공한다.
학교는 배정학교를 뜻하지 않고, 경사는 실제 보행 경사가 아니며, 후기 원문·평점은 공개하거나 순위에 쓰지 않는다. 미수집·오래됨·항목별 확인일 없음 상태를 함께 표시한다.</p>
<h2>2. 데이터 출처</h2>
...
후기 테마는 연결된 커뮤니티 집계 파일을 출처로 하며 각 화면에 관측·확인 상태를 별도로 표시한다.</p>
<h2>3. 신뢰 규율</h2>
