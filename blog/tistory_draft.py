...
def write_daily_draft(sections: list[str], today: str, data_asof: str, outdir: str = "report/blog") -> str:
    ...
    return write_helper(path, DraftPayload(title=title, tags=TISTORY_TAGS, body=body), today)
...
def write_digest_draft(digest: dict, today: str, outdir: str = "report/blog", kind: str | None = None) -> str:
    ...
    return write_helper(path, DraftPayload(title=title, tags=tags, body=body), today)
