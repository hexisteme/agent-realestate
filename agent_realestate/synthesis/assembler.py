...
.click-hint{font-size:11px;color:#4A6C8C;margin:2px 0 6px}"""
    ...
    # ★4차 감사 A(2026-06-13): 토지거래허가구역 설명 정정.
    # 구 텍스트 오류: "서울 전역, 2025.10.15~" — 토허는 서울 전역이 아니라 강남4구·목동·성수·잠실 등
    # 지정 구역에만 적용. 2025.10.15 는 스트레스DSR 강화 조치이지 토허 전역 확대가 아님.
    # Per-candidate 경고는 이제 Candidate.toher_zone=True + F_TOHER_RENT flag 로 분리(§5 플래그 컬럼 표시).
    _seoul = [e for e in ev if "서울" in (e.candidate.district or "")]
    _toher_flagged = [e for e in ev if any(getattr(f, "code", f) == "F_TOHER_RENT" for f in e.flags)]
    if _seoul:
        H.append("<div class='box' style='border-left:5px solid #B08020;background:#FAF5E8'>"
                 "<b>📋 토지거래허가구역 확인 필요 (서울 후보 해당)</b> — 강남4구·목동·성수·잠실 등 "
                 "<b>지정 구역</b>은(서울 전역 아님) <b>① 매수 시 허가 취득 필요 · ② 취득일로부터 2년 실거주의무</b>(이 기간 전세·임대 금지) · "
                 "<b>③ 자금조달계획서 의무</b>. "
                 "HOLD_AND_RENT 영향: '2년 실거주 후 임대' 타이밍 제약·즉시 갭투자 불가. "
                 f"후보 중 <b>토허구역 지정 단지({len(_toher_flagged)}건)</b>는 §5 F_TOHER_RENT 플래그로 표시됨. "
                 "미표시 후보도 단지 소재지가 지정구역인지 <b>land.seoul.go.kr 에서 직접 확인 권장</b>. "
                 "<span class=sub>출처: 토지거래허가구역 지정·운영 현황(서울시) · 정책브리핑 · "
                 "6.27 대책(소유권이전조건부 전세대출 금지 별도 게이트). 확인 2026-06-13.</span></div>")
